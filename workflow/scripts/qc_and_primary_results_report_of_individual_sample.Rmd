...
ref_rds_candidates <- c(
  "/data/BlueprintEncodeData.RDS",
  file.path(pipe_dir, "data/BlueprintEncodeData.RDS")
)
...
if(length(scMultiome[["SCT"]]@misc$DEGs_top5) > 0){
features_top5 <- unique(unlist(scMultiome[["SCT"]]@misc$DEGs_top5, use.names = FALSE))
features_top5 <- features_top5[!is.na(features_top5) & nzchar(features_top5)]
features_in_scale <- rownames(GetAssayData(scMultiome, assay = "SCT", layer = "scale.data"))
features_use <- intersect(features_top5, features_in_scale)

if (length(features_use) == 0) {
  message("Skipping DoMultiBarHeatmap: no top5 DEGs found in SCT scale.data.")
} else {
...
if(TRUE){
    ...
    deg_list <- lapply(scMultiome[["SCT"]]@misc$DEGs, id_convert)
    deg_list <- deg_list[lapply(deg_list, length) > 0]
    if (length(deg_list) > 0) {
      if (is.null(names(deg_list))) {
        names(deg_list) <- paste0("cluster_", seq_along(deg_list))
      }
      names(deg_list) <- make.unique(as.character(names(deg_list)))
